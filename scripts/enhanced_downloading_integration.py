...
class VerboseDownloadManager:
    ...
    def download_models(self):
        ...
        for i, model_name in enumerate(selected_models, 1):
            ...
            model_info = self.get_model_url(model_name, 'model')
            if model_info and isinstance(model_info, dict):
                model_url = model_info.get('url')
                model_filename = model_info.get('name', f"{model_name}.safetensors")
                model_path = self.base_path / 'models' / 'Stable-diffusion' / model_filename
        ...
        for i, vae_name in enumerate(selected_vaes, 1):
            ...
            vae_info = self.get_model_url(vae_name, 'vae')
            if vae_info and isinstance(vae_info, dict):
                vae_url = vae_info.get('url')
                vae_filename = vae_info.get('name', f"{vae_name}.safetensors")
                vae_path = self.base_path / 'models' / 'VAE' / vae_filename
        ...
        for i, lora_name in enumerate(selected_loras, 1):
            ...
            lora_info_list = self.get_model_url(lora_name, 'lora')
            if lora_info_list and isinstance(lora_info_list, list):
                for lora_info in lora_info_list:
                    if isinstance(lora_info, dict):
                        lora_url = lora_info.get('url')
                        lora_filename = lora_info.get('name', f"{lora_name}.safetensors")
                        lora_path = self.base_path / 'models' / 'Lora' / lora_filename
    ...
    def get_model_url(self, model_name, model_type):
        ...
        # Determine which model data file to use
        try:
            with open(self.settings_path, 'r') as f:
                settings = json.load(f)
            is_xl = settings.get('WIDGETS', {}).get('XL_models', False)
        except Exception:
            is_xl = False

        scripts_path = Path(os.environ.get('scr_path', '/content/LSDAI')) / 'scripts'
        model_data_file = scripts_path / ('_xl_models_data.py' if is_xl else '_models_data.py')
        ...
        try:
            ...
            if model_name in model_data:
                url_info = model_data[model_name]
                vprint(f"✅ Found URL info for {model_name}: {url_info}", VerbosityLevel.VERBOSE)
                return url_info
            else:
