...
def verify_dependencies():
    ...
    if missing:
        ...
        try:
            ...
            subprocess.run(['apt-get', 'install', '-y', *missing], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
        except (subprocess.CalledProcessError, FileNotFoundError) as e:
            print(f"Warning: Could not install some dependencies via apt-get: {e}")
            print("Please ensure git, curl, wget, aria2, lz4, and pv are installed.")
...
try:
    ...
    print(f"Fatal Error: Could not import custom modules: {e}. The setup in Cell 1 may have failed. Please restart the runtime and try again.")
    sys.exit(1)
...
def setup_venv():
    ...
    try:
        ...
        if widget_settings.get('detailed_download') == 'on':
            print(result.stdout)
        print("✅ Virtual environment dependencies are up to date.")
    except subprocess.CalledProcessError as e:
...
def handle_submodels(selection, num_selection, model_dict, dst_dir, base_url):
    ...
    unique_models = { (model.get('name') or os.path.basename(model['url'])): model for model in selected }.values()

    download_entries = [f"\"{model['url']}\" \"{dst_dir}\" \"{model.get('name', '')}\"" for model in unique_models]
    
    return (base_url + ", " if base_url else "") + ", ".join(download_entries)

# Process downloads
line = ""
line = handle_submodels(widget_settings.get('model', []), widget_settings.get('model_num', ''), model_list, webui_settings.get('model_dir', ''), line)
line = handle_submodels(widget_settings.get('vae', ''), widget_settings.get('vae_num', ''), vae_list, webui_settings.get('vae_dir', ''), line)
line = handle_submodels(widget_settings.get('lora', []), '', lora_list, webui_settings.get('lora_dir', ''), line)
line = handle_submodels(widget_settings.get('controlnet', []), widget_settings.get('controlnet_num', ''), controlnet_list, webui_settings.get('control_dir', ''), line)
    ...
    if empowerment_text:
        ...
        for line in lines:
            ...
            for prefix, (_, short_tag) in PREFIX_MAP.items():
                if line.startswith(short_tag) or line.lower() == prefix:
            ...
            if line.startswith('http') and current_prefix:
                ...
                custom_downloads.append(line) # Raw URL
else:
    custom_downloads.extend([
        widget_settings.get(key, '') for key in 
...
if custom_line:
    ...
    for link in filter(None, map(str.strip, custom_line.split(','))):
        ...
        if prefix:
            ...
            manual_download(link, SCR_PATH / "Downloads", prefix="misc")

if extension_repo:
    print(f"✨ Installing custom extensions...")
    for repo_url, repo_name in extension_repo:
        m_clone(f"{repo_url} {webui_settings.get('extension_dir', '')} {repo_name or ''}")
